...
class CallGraph:
    ...
    # Hard limit for speculative expansion to prevent state explosion
    MAX_SPECULATIVE_CANDIDATES = 5
    ...
    def get_potential_callees(
        self, method_name: str, limit: Optional[int] = None
    ) -> List[str]:
        """
...
class CallFinder(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call):
        call_info = {"node": node, "kind": "unknown", "name": None, "base": None}
    ...
    def _process_statement(self, caller: str, stmt: ast.AST):
        ...
        for call in finder.calls:
            if call["kind"] == "function":
                ...
                # Add edges to ALL classes that implement this method (up to a limit).

                candidates = self.cg.get_potential_callees(
                    method_name, limit=self.cg.MAX_SPECULATIVE_CANDIDATES
                )
