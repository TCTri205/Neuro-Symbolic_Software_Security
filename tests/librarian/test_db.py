...
def test_init_db(librarian_db, db_path):
    ...
    # Check tables exist
    tables = ["decisions", "vulnerability_types", "remediation_strategies", "library_profiles"]
    for table in tables:
...
def test_knowledge_graph_population(librarian_db):
    ...
    cursor.execute("SELECT * FROM remediation_strategies WHERE vulnerability_type_id = 'test.vuln'")
    rem = cursor.fetchone()
...
def test_store_decision_updates(librarian_db):
    ...
    # Update same hash
    librarian_db.store_decision(context_hash, [{"verdict": "VULNERABLE"}], "raw2", "model2")
